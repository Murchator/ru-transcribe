...
export async function decodeToMono16k(file, onProgress) {
  ...
  if (!AC) throw new Error("This browser can't decode audio. Try Chrome, Edge, Safari or Firefox.");
  ...
  try {
    ...
    throw new Error("Couldn't read that audio. Try converting it to MP3, M4A or WAV first.");
  } finally {
    ctx.close?.();
  }
